...
workflow_src <- file.path("..", "input", "workflow.excalidraw.png")
workflow_dst <- file.path("assets", "workflow.excalidraw.png")

if (file.exists(workflow_src)) {
  dir.create(dirname(workflow_dst), recursive = TRUE, showWarnings = FALSE)
  file.copy(workflow_src, workflow_dst, overwrite = TRUE)
}
```
...
<div class="workflow-figure">
  <img src="assets/workflow.excalidraw.png" alt="VirMarkDB workflow overview"
    onerror="this.style.display='none'; this.parentElement.classList.add('missing-workflow-img'); this.parentElement.innerHTML='<p><strong>Workflow figure not found.</strong><br>Copy <code>input/workflow.excalidraw.png</code> to <code>docs/assets/workflow.excalidraw.png</code> before rendering.</p>';">
</div>
```
...
```text
scripts/database_generation/02_prodigal.bash
```
...
```text
scripts/database_generation/03_align_markers.bash
```
...
```text
scripts/database_generation/04_hmmbuild.bash
```
...
```text
scripts/database_generation/05_hmmsearch.bash
```
